...
class _SalonFetchAPIDataState extends State<SalonFetchAPIData> {
  final SalonsDetailAPI _apiService = SalonsDetailAPI();
  int? _salonId;
  late Future<SalonResponseData?> _salonDataFuture;

  @override
  void didChangeDependencies() {
    super.didChangeDependencies();
    _salonId = ModalRoute.of(context)?.settings.arguments as int? ?? 1;
    _salonDataFuture = _apiService.fetchSalonData(_salonId!);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: FutureBuilder<SalonResponseData?>(
          ...
          builder: (context, snapshot) {
            ...
            final salonResponse = snapshot.data!;
            developer.log('SalonResponseData: ${salonResponse.toJson()}');
            if (salonResponse.response == null) {
              developer.log('SalonResponseData.response is null');
              return Center(
                child: Column(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
            ...
            if (salonResponse.response!.data == null) {
              developer.log('SalonResponseData.response.data is null');
              return Center(
                child: Column(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
            ...
            final salonData = salonResponse.response!.data!;
            return Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                // Salon Images
                if (salonData.images?.isNotEmpty ?? false)
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Text(
                          ...
                          child: ClipRRect(
                            ...
                            child: Image.network(
                              ...
                              errorBuilder: (context, error, stackTrace) {
                                developer.log('Failed to load image: $imageUrl, error: $error');
                                return const Text(
                      ...
                      children: [
                        ...
                        if (section.data?.isNotEmpty ?? false)
                          ...section.data!.map((item) {
                            ...
                            // Reviews (type: 3)
                            else if (section.type == '3' && item is ReviewData) {
                              return Card(
                                margin: const EdgeInsets.only(bottom: 10),
                                child: ListTile(
                            ...
                            // Staff (type: 4)
                            else if (section.type == '4' && item is Professional) {
                              return Card(
                                margin: const EdgeInsets.only(bottom: 10),
                                child: ListTile(
                                  title: Text(item.name ?? 'N/A'),
                                  subtitle: Text('Experience: ${item.experience ?? 'N/A'}'),
                                  leading: item.image != null
                                      ? CircleAvatar(
                                    backgroundImage: NetworkImage(item.image!),
                                    onBackgroundImageError: (error, stackTrace) {
                                      developer.log('Failed to load staff image: ${item.image}, error: $error');
                                    },
                                  )
                                ...
                                child: Padding(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      const SizedBox(height: 8),
                                      if (item.opening_timings?.isNotEmpty ?? false) ...[
                                        const Text(
                                          'Opening Hours:',
                                          style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold),
                                        ),
                                        ...item.opening_timings!.map((timing) => Text(
                            ...
                            developer.log('Unknown section item: $item, type: ${section.type}');
