...
import 'package:app/screens/home/components/deals_dashboard.dart';
...
import '../../components/sale_amount.dart';
...
class SalonCategoryAndServicesList extends StatefulWidget {
  ...
  @override
  _SalonCategoryAndServicesListState createState() => _SalonCategoryAndServicesListState();
}

class _SalonCategoryAndServicesListState extends State<SalonCategoryAndServicesList> {
  ...
  void _handleAddToCart(dynamic item) {
    setState(() {
      ...
      _totalAmount = _cartItems.entries.fold(0.0,
  ...
  Future<void> loadData() async {
    ...
    if(mDeal != null){
      responseData = await api.fetchAllServicesAndDealsCategorizedData(s as String, mDeal?.salon?.id ?? 0 );
      log("::::mDeal?.services?[0]?.salon?.id  ${mDeal?.salon?.id }");
    }else{
      responseData = await api.fetchAllServicesAndDealsCategorizedData(s as String, mSericve?.salon?.id ?? 0 );
      log("::::mSericve?.services?[0]?.salon?.id  ${mDeal?.salon?.id }");
    }



    print('responseData?.response?.data?.length:0: ${responseData?.response?.data?.length}');
                              ...
                              Material(
                                ...
                                child: InkWell(
                                  ...
                                  child: AnimatedContainer(
                                    ...
                                    decoration: BoxDecoration(
                                      color: _cartItems.containsKey(item) ? kPrimaryDarkColor : kScreenBg,
                                      borderRadius: BorderRadius.circular(30),
                                      boxShadow: [
                                        BoxShadow(
                                    ...
                                    child: Text(
                                      _cartItems.containsKey(item) ? 'Added' : 'Add',
                                      style: TextStyle(
                                        color: _cartItems.containsKey(item) ? Colors.white : kPrimaryDarkColor,
                                        fontWeight: FontWeight.bold,
                                        fontSize: 14,
                                      ),
                                    ),
